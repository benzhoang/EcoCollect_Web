...
import { getCollectorAssignmentReportDetail, getWasteCategories } from "../../service/api";
...
const mapDetailToRequest = (raw, categoryMap = {}) => {
  ...
  const status = report?.currentStatus ?? data?.currentStatus ?? data?.status ?? report?.status ?? "-";
  ...
  return {
    ...
    wasteType,
    wasteTypeColor:
      report?.wasteTypeColor ?? data?.wasteTypeColor ?? getWasteTypeColor(wasteType),
    estimatedWeight:
    ...
    assignedAt: data?.assignedAt ?? report?.assignedAt ?? data?.createdAt ?? "-",
    status,
    statusColor: data?.statusColor ?? report?.statusColor ?? getStatusColor(status),
        ...
        <div className="grid grid-cols-1 gap-6 mb-6 lg:grid-cols-2">
          ...
          <div className="p-5 bg-white border border-gray-200 rounded-xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                <p className="mb-1 text-xs font-semibold tracking-wide text-gray-500 uppercase">Mô tả</p>
                <p className="font-medium text-gray-900 break-words">{request.description}</p>
              </div>
              <div>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                <div>
                  <p className="mb-1 text-xs font-semibold tracking-wide text-gray-500 uppercase">Vĩ độ</p>
                  <p className="font-medium text-gray-900">{request.latitude}</p>
                </div>
                <div>
                  <p className="mb-1 text-xs font-semibold tracking-wide text-gray-500 uppercase">Kinh độ</p>
                  <p className="font-medium text-gray-900">{request.longitude}</p>
                </div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {images.slice(0, 4).map((img, i) => (
                    <div key={i} className="h-40 overflow-hidden bg-gray-100 border border-gray-200 rounded-lg">
